...
public class Place_Train_Choice extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.place_train_choice);

        get_save_set_save_day();

        start_station = (TextView)findViewById(R.id.start_station);
        start_station.setText(getIntent().getStringExtra("nodename")+"역에서 출발");
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(Place_Train_Choice.this);
        //   linearLayoutManager.setStackFromEnd(true); //포커스가 마지막 포지션으로 감
        recyclerview.setLayoutManager(linearLayoutManager);
        recyclerview.setHasFixedSize(true);
        arrayList = new ArrayList<>();
        adapter= new CusTomAdapter_Train_Choice_list(arrayList,Place_Train_Choice.this);
        recyclerview.setAdapter(adapter);

        database=FirebaseDatabase.getInstance("https://oneplace-db16a-default-rtdb.firebaseio.com/");
        databaseReference=database.getReference("-기차역").child("-기차역매칭LIST").child(getIntent().getStringExtra("nodename"));
